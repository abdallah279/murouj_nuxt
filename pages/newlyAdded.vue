<template>
    <main class="about_page main-padding">
        <div class="container">
            <PageHeader :title="$t('pagesTitle.newlyAdded')" margin="mb-5" />
...
const getData = async () => {
    loading.value = true;
    await axios.get(`latest-products?country_id=${countryID.value}&page=${currentPage.value}`, config.value).then(res => {
